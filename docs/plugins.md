...
| 🗓️ | [Outlook Work Calendar](#🗓️-outlook-work-calendar) | Fetches published Outlook work calendar via EWS JSON API (no auth required) | 1 |
...
#### config_backup_run
**Description:** Back up OpenClaw config and agent workspace to Git. Copies `~/.openclaw` config files into the Git repo, commits, and pushes only when content has changed (SHA-256 detection).
...
**Description:** Take a snapshot from a Home Assistant camera. Saves the image locally and returns the file path. Use camera_name 'all' to capture every camera.
...
#### hass_camera_list
**Description:** List all available Home Assistant cameras and their entity IDs.
...
#### ha_state_get
**Description:** Get the current state of a Home Assistant entity. Returns attributes, state value, and last-changed timestamp.
...
#### ha_state_list
**Description:** List all Home Assistant entities, or filter by domain (e.g. light, switch, sensor, camera, person).
...
#### ha_service_call
**Description:** Call a Home Assistant service (e.g. turn on a light, activate a scene). Specify domain, service, and optionally an entity_id and extra data.
...
#### ha_event_list
**Description:** List recent Home Assistant events, optionally filtered by entity_id.
...
#### ics_calendar_fetch
**Description:** Fetch upcoming events from Nicole's ICS calendar feed. Requires the CALENDAR_NICOLE_ICS_URL environment variable.
...
#### opentable_lookup
**Description:** Look up an OpenTable restaurant by its URL slug (e.g. 'carbone-new-york' from opentable.com/r/carbone-new-york) to get its numeric restaurant ID.
...
**Description:** Check real-time availability for a restaurant on OpenTable. Returns available time slots with booking URLs.
...
**Description:** Run OpenTable health check. Returns status (ok or error) and a message.
...
**Description:** Fetch upcoming events from personal and/or family Outlook calendars via Microsoft Graph API.
...
#### outlook_inbox
**Description:** List recent messages from the Outlook inbox.
...
**Description:** Fetch upcoming events from the published Outlook work calendar. Uses the EWS JSON API — no authentication required. Requires the OUTLOOK_WORK_CALENDAR_URL environment variable.

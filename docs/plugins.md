...
Below is a list of available plugins, each with a brief description. Click on a plugin name to jump to its detailed section.
...
- [OpenTable Heartbeat](#opentable-heartbeat): Health-check for the OpenTable skill. Alerts on failure via configured notification channel.
...
The documentation continues with similar sections for each plugin. If you'd like me to complete the remaining plugins, let me know!

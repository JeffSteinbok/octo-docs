...
1. **`session-labels.csv`** — Written by `enrich_sessions.py` (daily at 4 AM via `usage-enrich-sessions` cron). Provides high/medium confidence labels using heuristics + optional LLM fallback.

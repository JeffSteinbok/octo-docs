...
This page provides an overview of scheduled jobs that automate recurring tasks. These jobs help streamline operations by performing actions such as calendar fetching, health checks, reminders, and backups at specified times. Each job is configured with a schedule and purpose to ensure timely execution and reliability.
...
1. Jobs are defined with a name, description, and schedule.
2. Each job is enabled or disabled based on configuration.
3. Schedules use either cron expressions, fixed intervals, or one-shot timing.
4. When the scheduled time arrives, the job executes its defined task.
5. Some jobs perform checks and send notifications if issues are detected.

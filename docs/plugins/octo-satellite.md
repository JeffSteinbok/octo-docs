...
> **Source:** [JeffSteinbok/openclaw-hub](https://github.com/JeffSteinbok/openclaw-hub/tree/main/plugins/octo-satellite)
...
Get sync status for all linked Monarch Money accounts — last synced time, institution health, and connection state for each account.

### `monarch_refresh_accounts`

Trigger an account refresh with all linked Monarch Money institutions. Fire-and-forget — returns immediately after requesting the refresh. Use monarch_get_sync_status to check progress afterward.
...
## Verify Monarch Money session is authenticated and the connection is healthy.
node dist/bin/octo-satellite.js monarch-get-health

## Get sync status for all linked Monarch Money accounts — last synced time, institution health, and connection state for each account.
node dist/bin/octo-satellite.js monarch-get-sync-status

## Trigger an account refresh with all linked Monarch Money institutions. Fire-and-forget — returns immediately after requesting the refresh. Use monarch_get_sync_status to check progress afterward.
node dist/bin/octo-satellite.js monarch-refresh-accounts

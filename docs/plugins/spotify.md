...
| `device_id` | string | Optional | Target device ID (from spotify_get_devices). Omit to use the active device. |
...
| `type` | string | Optional | Type of result to search for (default: track). Allowed: `track`, `album`, `artist`, `playlist`. |

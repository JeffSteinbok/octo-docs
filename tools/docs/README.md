...
Documentation is generated by consuming a **sanitized bundle** produced by the private `octo` repo, running it through an LLM API, and writing polished Markdown pages.

This system does **not** access the private source repository directly.

Selected public plugins, services, and shared Python libs may also be mirrored into `openclaw-hub`, but this generator still reads only the sanitized bundle artifact emitted by `octo`.
